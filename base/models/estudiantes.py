...
class Estudiante:
    ...
    def get_all(cls):
        ...
        estudiantes = []
        if results:
            for estudiante_data in results:  # type: ignore
                estudiantes.append(cls(estudiante_data))
        return estudiantes
    ...
    def validate_estudiante(estudiante):
        ...
        result = connectToMySQL(DB_NAME).query_db(query, data)
        if result and result[0]['id'] != int(estudiante['id']):  # type: ignore # Permite editar sin error si el email no cambia
